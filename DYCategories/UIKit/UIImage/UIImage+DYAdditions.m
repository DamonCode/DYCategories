...
#import "UIImage+DYAdditions.h"
#import <CoreText/CoreText.h>
#import <ImageIO/ImageIO.h>
#import <Accelerate/Accelerate.h>
#import "DYUtilities.h"
#import "DYCategoriesMacro.h"
@implementation UIImage (DYAdditions)
+ (NSCache *)dy_cache{
...
CGRect DYCGRectFitWithContentMode(CGRect rect, CGSize size, UIViewContentMode mode) {
...
+ (UIImage *)imageWithPDF:(id)dataOrPath {
    return [self dy_imageWithPDF:dataOrPath resize:NO size:CGSizeZero];
}
+ (UIImage *)imageWithPDF:(id)dataOrPath size:(CGSize)size {
    return [self dy_imageWithPDF:dataOrPath resize:YES size:size];
}
...
+ (UIImage *)imageWithFont:(UIFont *)font named:(NSString *)iconNamed
             withTintColor:(UIColor *)tintColor clipsToBounds:(BOOL)clipsToBounds fontSize:(CGFloat)fontSize {
    NSString *identifier = [NSString stringWithFormat:@"%@%@%@%@%d%f", NSStringFromSelector(_cmd), font.fontName, tintColor, iconNamed, clipsToBounds, fontSize];
    UIImage *image = [[self dy_cache] objectForKey:identifier];
    if(image == nil){
...
#pragma clang diagnostic ignored "-Wundeclared-selector"
            if(clipsToBounds && [image respondsToSelector:@selector(imageClippedToPixelBounds)]){
                image = [image performSelector:@selector(imageClippedToPixelBounds)];
            }
#pragma clang diagnostic pop
            
            [[self dy_cache] setObject:image forKey:identifier];
        }
    }
    return image;
}
...
+ (UIImage *)dy_imageWithPDF:(id)dataOrPath resize:(BOOL)resize size:(CGSize)size {
...
- (UIImage *)transparentBorderImage:(NSUInteger)borderSize {
    ...
    CGImageRef maskImageRef = [self dy_newBorderMask:borderSize size:newRect.size];
...
- (CGImageRef)dy_newBorderMask:(NSUInteger)borderSize size:(CGSize)size {
...
- (void)drawInRect:(CGRect)rect withContentMode:(UIViewContentMode)contentMode clipsToBounds:(BOOL)clips{
    CGRect drawRect = DYCGRectFitWithContentMode(rect, self.size, contentMode);
...
- (nullable UIImage *)imageByInsetsEdge:(UIEdgeInsets)insets withColor:(UIColor *)color {
...
- (UIImage *)imageByRotate180 {
    return [self dy_flipHorizontal:YES vertical:YES];
}

- (UIImage *)imageByFlipVertical {
    return [self dy_flipHorizontal:NO vertical:YES];
}

- (UIImage *)imageByFlipHorizontal {
    return [self dy_flipHorizontal:YES vertical:NO];
}

- (UIImage *)dy_flipHorizontal:(BOOL)horizontal vertical:(BOOL)vertical {
...
- (UIImage *)imageByBlurRadius:(CGFloat)blurRadius
    ...
    if (self.size.width < 1 || self.size.height < 1) {
        NSLog(@"UIImage+DYAddition error: invalid size: (%.2f x %.2f). Both dimensions must be >= 1: %@", self.size.width, self.size.height, self);
        return nil;
    }
    if (!self.CGImage) {
        NSLog(@"UIImage+DYAddition error: inputImage must be backed by a CGImage: %@", self);
        return nil;
    }
    if (maskImage && !maskImage.CGImage) {
        NSLog(@"UIImage+DYAddition error: effectMaskImage must be backed by a CGImage: %@", maskImage);
        return nil;
    }
    ...
    if (!hasBlur && !hasSaturation) {
        return [self dy_mergeImageRef:imageRef tintColor:tintColor tintBlendMode:tintBlendMode maskImage:maskImage opaque:opaque];
    }
    ...
    if (hasNewFunc) {
        ...
        effectCGImage = vImageCreateCGImageFromBuffer(input, &format, &dy_cleanupBuffer, NULL, kvImageNoAllocate, NULL);
        if (effectCGImage == NULL) {
            effectCGImage = vImageCreateCGImageFromBuffer(input, &format, NULL, NULL, kvImageNoFlags, NULL);
            free(input->data);
        }
        free(output->data);
        outputImage = [self dy_mergeImageRef:effectCGImage tintColor:tintColor tintBlendMode:tintBlendMode maskImage:maskImage opaque:opaque];
...
- (UIImage *)dy_mergeImageRef:(CGImageRef)effectCGImage
                    tintColor:(UIColor *)tintColor
...
static void dy_cleanupBuffer(void *userData, void *buf_data) {
    free(buf_data);
}
